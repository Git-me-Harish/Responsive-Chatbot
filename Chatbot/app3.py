...
# Set API keys
GROQ_API_KEY = "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"
LLAMA_PARSE_API_KEY = "LL-2SvQfgKw12DLJ9bTboz9SiK9x5z177EOCY81escll4Bm8P5gSrloDDhFXP9x2ynD"
...
async def initialize_components():
    ...
    llama_parse_documents = await parser.aload_data("resume.pdf")
...
def query_answer(query, qa):
    response = qa.invoke(query)
    return markdown.markdown(response["result"])

async def gradio_interface():
    qa = await initialize_components()

    interface = gr.Interface(
        fn=query_answer,
        inputs=["text", qa],
        outputs="markdown",
        title="NVIDIA Financial Results Q&A",
        description="Ask questions about NVIDIA's First Quarter Fiscal 2024 Financial Results.",
    )

    interface.launch()

if __name__ == "__main__":
    asyncio.run(gradio_interface())